...
const FavoriteRecipes = ({isComponent}) => {
    ...
    function getMyFavorites() {
        api.get(`/recipe/favorites/?skip=${skip}&take=${take}&${query}`)
            .then((response) => {
                setRecipes(response.data);
            })

        api.get(`/recipe/favorites-count/?&${query}`)
            .then((response) => {
    ...
    function onApplyFilters(filterByCreators, filterByIngredients, filterByCuisines, filterByDiets, filterByCookingTime, filterByName) {
        ...
        api.get(`/recipe/favorites/?skip=${skip}&take=${take}&${queryString}`)
            .then((response) => {
                setRecipes(response.data);
            })

        api.get(`/recipe/favorites-count/?${queryString}`)
            .then((response) => {
                let total = response.data.count;
                setTotalItems(total)
                let numberOfPages = Math.ceil(Number(total) / Number(take))
    ...
    function breadCrumbs() {
        if (!isComponent) {
            return (
                <>
                    <BreadCrumbs basePage="Home" currentPage="My Favorites"></BreadCrumbs>
